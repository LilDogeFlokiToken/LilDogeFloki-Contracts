...
contract LilDogeFloki is Context, IERC20, Ownable {
    ...
    string private _name = "Lil Doge Floki";
    ...
    // @dev set Pair
    function setPair(address _pancakeswapPair) external onlyOwner {
        pancakeswapPair = _pancakeswapPair;
    }

    // @dev set Router
    function setRouter(address _newPancakeswapRouter) external onlyOwner {
        IPancakeswapRouter02 _pancakeswapRouter = IPancakeswapRouter02(_newPancakeswapRouter);
    ...
    function expectedRewards(address _sender) external view returns(uint256){
        ...
        uint256 rewards = holdersBal.mul(_balance).div(_tTotal.sub(balanceOf(pancakeswapPair)).sub(totalExcludedBal));
        return rewards;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for BNB
        swapTokensForBnb(half.add(devExp).add(forRewards)); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered
